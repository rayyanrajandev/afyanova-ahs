...
/**
 * Raises a service charge for a pharmacy / medication prescription order.
 *
 * Enforces the prepaid rule for pharmacy: a charge is created the moment
 * the clinician prescribes the medicine, before any drug is prepared or
 * dispensed. The order remains pending payment until settled by the cashier.
 *
 * Multiplies unit tariff by quantity_prescribed.
 *
 ...
 * the clinician's prescription workflow.
 */
class PharmacyChargeRaiser
{
    ...
    public function raiseFor(array $order, ?int $actorUserId = null): ?ServiceChargeModel
    {
        try {
            ...
            Log::warning('Unable to raise service charge for pharmacy order.', [
    ...
    {
        if (! (bool) config('revenue.prepaid_required_for.pharmacy_order', true)) {
            return null;
        }
        ...
        if (! $payerClass->isImplemented()) {
            Log::info('Pharmacy order left uncharged: payer class is not settled in this phase.', [
        ...
        if ($chargeableItem === null) {
            Log::warning('Pharmacy order left uncharged: no chargeable item configured for medication.', [
                'pharmacy_order_id' => $orderId,
                'medication_code' => $order['medication_code'] ?? null,
                'catalog_item_id' => $order['approved_medicine_catalog_item_id'] ?? null,
            ]);
        ...
        $quantity = (float) ($order['quantity_prescribed'] ?? 1.0);
        if ($quantity <= 0) {
            $quantity = 1.0;
        }

        $unit = (string) ($order['prescribed_unit'] ?? $chargeableItem->default_unit ?? 'unit');
        ...
        return $this->raiseServiceCharge->execute(
            ...
            description: (string) ($order['medication_name'] ?? $chargeableItem->name),
            quantity: $quantity,
    ...
    /**
     * Resolve the ChargeableItemModel for the prescribed medication.
     *
     * @param  array<string, mixed>  $order
     */
    private function resolveChargeableItem(array $order): ?ChargeableItemModel
    {
        ...
        $medicationCode = strtoupper(trim((string) ($order['medication_code'] ?? '')));
        ...
        // 2. Match by medication code
        if ($medicationCode !== '') {
            $item = ChargeableItemModel::query()
                ->whereRaw('UPPER(code) = ?', [$medicationCode])
                ->where('status', 'active')
                ->where(fn ($q) => $q->whereNull('facility_id')->orWhere('facility_id', $facilityId))
                ->orderByRaw('CASE WHEN facility_id IS NULL THEN 1 ELSE 0 END')
                ->first();
