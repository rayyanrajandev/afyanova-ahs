...
export interface UseRadiologyLiveSyncOptions {
  /** Refetch active radiology worklist when queue updates. */
  onQueueUpdated: () => void;
}

export function useRadiologyLiveSync(options: UseRadiologyLiveSyncOptions): void {
  const page = usePage();
  ...
  try {
    useEcho(`radiology-queue.${facilityId}`, ".queue.updated", () => {
